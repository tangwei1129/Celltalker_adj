...
cell_type_ligand_receptor_score <- function(replicate_scores_frame,
  ...
  for (i in 1:nrow(interactions_compare)) {
    ...
    sample1 <- suppressMessages(extracted_sample_groups %>% left_join(sample1,by=c(sample_groups,sample_replicates)))
    ...
    sample2 <- suppressMessages(extracted_sample_groups %>% left_join(sample2,by=c( sample_groups , sample_replicates)))
    ...
    sample_score <- data.frame((sample1[,3] + sample2[,3]) / 2,
                               ...
                               sample_replicates=sample1[,sample_replicates],
...
compare_group_interactions_adj <- function(seurat_object,adj_var="nFeature_RNA", 
                                           interaction_stats,
                                           sample_replicates,
                                           sample_groups,
                                           metadata_grouping) {
  ...
  # add adj_var back to interaction_scores_frame
  temp <- seurat_object[[c(sample_replicates,adj_var)]] %>% as_tibble() %>% distinct()
  
  colnames(temp) <- c("sample_replicates", adj_var) 
  interaction_scores_frame <- interaction_scores_frame %>% left_join(temp, by = "sample_replicates")
  
  # add adj_var to lm function
  input_formula <- stats::as.formula(paste("scores~sample_groups + ", paste(adj_var, collapse  = "+"),sep=""))  
  mod_res <- interaction_scores_frame %>%
    mutate(unique_group=paste(cell_type1,ligand,cell_type2,receptor,sep="_")) %>%
    split(.$unique_group) %>%
    map(~lm(input_formula,data=.))
